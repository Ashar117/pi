...
def test_get_system_prompt_includes_mode_block_root():
    ...
    assert any(s in p for s in ("TOOLS: All 7 ENABLED", "TOOLS: All", "TOOLS: ENABLED")), "root tool note missing"
...
def test_get_system_prompt_includes_consciousness_text():
    """T-062: root uses full consciousness.txt (header 'PI CONSCIOUSNESS'),
    normie uses the slim consciousness_normie.txt (header 'You are Pi').
    ...
    assert "PI CONSCIOUSNESS" in p_root, "root consciousness text not in system prompt"

    a.mode = "normie"
    p_normie = a._get_system_prompt()
    assert "You are Pi" in p_normie, "normie consciousness text not in system prompt"
