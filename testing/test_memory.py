...
def test_001_memory_read_after_write():
    ...
    # UUID-first so dedup (hash + word-overlap) never fires across runs
    run_id = uuid.uuid4().hex
    test_content = f"t001_rw_{run_id}"
    ...
    read_result = memory.memory_read(query=run_id)
    ...
    found = any(run_id in str(entry) for entry in read_result)
    assert found, f"Unique token not found in read results. Results: {read_result}"
...
def test_001_memory_bulk_read():
    ...
    run_id = uuid.uuid4().hex
    test_items = [
        f"t001_bulk_alpha_{run_id}",
        f"t001_bulk_beta_{run_id}",
        f"t001_bulk_gamma_{run_id}",
    ]
...
def test_001_single_fact_recall():
    ...
    # UUID-first content — dedup never fires, and we assert on the UUID itself
    run_id = uuid.uuid4().hex
    fact_content = f"t001_fact_{run_id}_deadline_march15"
    memory.memory_write(
    ...
    result = memory.memory_read(query=run_id)
    ...
    found = any(run_id in str(entry) for entry in result)
...
def test_001_write_read_l3_vs_l2():
    ...
    NOTE: Under pytest this test would write real rows to prod L2
    (organized_memory) — the L1/L2 write guard now blocks that, which makes
    the read assertion fail. Skip under pytest; the manual __main__ runner
    still exercises it against a dev Supabase if needed.
    """
    if os.environ.get("PYTEST_CURRENT_TEST"):
        import pytest
        pytest.skip("writes real L2 rows — run manually via __main__ if needed")
