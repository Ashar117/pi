...
def _gmail_inst():
    """Lazy GmailTools accessor. Heavy google-api import deferred to first call."""
    return GmailTools()


def _handle_gmail_inbox(agent, tool_input, *, memory_override=None):
    return _gmail_inst().inbox_summary(max_results=tool_input.get("max_results", 5))


def _handle_gmail_search(agent, tool_input, *, memory_override=None):
    return _gmail_inst().gmail_search(
        query=tool_input["query"],
        max_results=tool_input.get("max_results", 10),
    )


def _handle_gmail_read(agent, tool_input, *, memory_override=None):
    return _gmail_inst().gmail_read(message_id=tool_input["message_id"])


def _handle_gmail_send(agent, tool_input, *, memory_override=None):
    return _gmail_inst().gmail_send(
        to=tool_input["to"],
