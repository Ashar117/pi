...
# ── Emit prompts/capabilities.md (T-050) ────────────────────────────────────

def emit_capabilities_md() -> bool:
    """Auto-generate prompts/capabilities.md from live tool definitions.

    Returns True if the file changed (or was created), False if unchanged.
    """
    try:
        from agent.tools import get_tool_definitions
        tools = get_tool_definitions()
    except Exception as e:
        print(f"[refresh_pi] capabilities.md skipped — agent.tools failed: {e}", file=sys.stderr)
        return False

    lines: List[str] = [
        "# Pi Capabilities",
        "",
        f"*Auto-generated — {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')} · {len(tools)} tools*",
        "",
    ]
    for t in sorted(tools, key=lambda x: x["name"]):
        desc = (t.get("description") or "").split("\n")[0][:120]
        props = list((t.get("input_schema") or {}).get("properties", {}).keys())
        args_str = f"({', '.join(props)})" if props else "()"
        lines.append(f"- **{t['name']}**{args_str} — {desc}")

    lines.append("")
    new_text = "\n".join(lines)

    existing = CAPABILITIES_MD.read_text(encoding="utf-8") if CAPABILITIES_MD.exists() else ""
    # Strip the timestamp line for comparison (it always changes)
    def _strip_ts(s: str) -> str:
        return re.sub(r"\*Auto-generated[^\n]*\n", "", s)

    if _strip_ts(existing) == _strip_ts(new_text):
        return False

    CAPABILITIES_MD.parent.mkdir(parents=True, exist_ok=True)
    CAPABILITIES_MD.write_text(new_text, encoding="utf-8")
    return True
...
def main() -> int:
    ...
    # T-050: emit capabilities.md alongside PI.md
    caps_changed = emit_capabilities_md()

    if updated == original:
        if caps_changed:
            print("[refresh_pi] PI.md already up to date. capabilities.md regenerated.")
        else:
