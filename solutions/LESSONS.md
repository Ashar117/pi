...
## L-014 — Pretty Unicode in stdout-printed strings is a Windows portability landmine

**Date:** 2026-05-02
**From:** T-022, S-015, [FINDINGS.md F-002](../FINDINGS.md)
**Lesson:** `python testing/test_requirements.py` raised `UnicodeEncodeError: 'charmap' codec can't encode character '✓'` on a fresh Windows shell (Python 3.13, default cp1252 stdout). The check-mark and cross-mark characters (`✓` U+2713, `✗` U+2717) and the box-drawing chars (`═` U+2550) used across pi_agent.py, agent/health.py, and 6 testing/ scripts all worked silently for the developer (whose shell had `PYTHONIOENCODING=utf-8` set globally) but crashed on a clean shell or a fresh CI runner. The env-var-check logic itself was fine; only the *printout* crashed. The bug had been latent for an unknown number of weeks because every test session happened to inherit a UTF-8-capable stdout.
**Rule:** Use ASCII-safe sigils (`[OK]`, `[FAIL]`, `[WARN]`) in any string that gets `print()`-ed by a script that might be launched outside your usual shell — tests, CLI tools, and anything CI runs. Reserve Unicode glyphs for strings that stay in-memory (system prompts, JSON content, message bodies). When you really want box-drawing aesthetics in a script's stdout, put `sys.stdout.reconfigure(encoding="utf-8", errors="replace")` at the top of the entry point — but only the entry point; scripts launched independently won't inherit it. Verify portability with `PYTHONIOENCODING=cp1252 python <script>` on the script's own shebang.
